...
def TOTAL_FEATURES(trace_data, max_size=175):
    ...
    # ------TIME--------
    result = interarrival_maxminmeansd_stats(list_data)
    if len(result) == 0:
        return np.zeros(max_size, dtype=float)
    intertimestats = [x for x in result[0]]
    ...
    avg_per_sec, std_per_sec, med_per_sec, min_per_sec, max_per_sec, per_sec = number_per_sec(
        trace_data
    )
...
def extract_dill(traces):
    """Extract Features -- A dictionary containing features for each traffic instance."""
    extracted_features = []
    for i in tqdm(range(len(traces)), desc="Extracting KFP Features"):
        g = []
        trace = traces[i].copy()
        trace[:, 0] = trace[:, 0] - trace[0, 0]
        g = TOTAL_FEATURES(trace)
        # 提取特征时不足补0
        while len(g) < feature_number:
            g.append(0)
        extracted_features.append(g)

    # extracted_features = Parallel(40, backend="multiprocessing")(
    #     delayed(TOTAL_FEATURES)(feature) for feature in tqdm(features)
    # )
    # for feature in extracted_features:
    #     if len(feature) < feature_number:
    #         feature.extend([0] * (feature_number - len(feature)))
...
class KFP(MLAttack):
    ...
    def init_model(self):
        self.model = RandomForestClassifier(n_jobs=-1, n_estimators=1000, oob_score=True, verbose=1)

    def data_preprocess(self, traces, labels):
        return {"traces": np.array(extract_dill(traces)), "labels": np.array(labels)}
